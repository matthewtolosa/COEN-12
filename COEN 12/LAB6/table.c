...
//Big O: O(n);
static int findPivot(SET *sp, void **elts, int lo, int hi)
{

assert(sp!=NULL); 
void *initialPivot;
initialPivot = elts[hi];
int smallIdx, i;
smallIdx = lo - 1;

	for(i = lo; i<hi -1; i++)
	{
		if((*sp->compare)(elts[i],initialPivot) <= 0)	
		{
			smallIdx++;
			void *temp;
			temp = elts[smallIdx];
			elts[smallIdx] = elts[i];
			elts[i] = temp;
		}
	}
	void *swap = elts[smallIdx + 1];
	elts[smallIdx + 1] = elts[hi];
	elts[hi] = swap;
	return smallIdx + 1;
}
...
static void quickSort(SET *sp, void **elts, int lo, int hi)
{
	...
	{
		int returnIdx;
		returnIdx =  findPivot(sp, elts, lo, hi);
		quickSort(sp, elts, lo, returnIdx - 1);
		quickSort(sp, elts, returnIdx + 1, hi);
	}
	return;
}
